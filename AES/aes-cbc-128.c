...
void aes_cbc_128(unsigned char *plaintext){
	...
	for(j = 0; j < 16; j++)
        ...
        XOR(block_message, round_iv);

	//Print Input Block
	printf("\nInput block %d : ", k+1);
        for(i = 0;i < 16;i++)
            printf("%2.2x",block_message[i]);
	aes_encrypt(key, block_message, expandedkey, &ciphertext[k*16]);
    ...
    for(i = 0;i < 16;i++)
        printf("%2.2x",block_message[i]);
    aes_encrypt(key, block_message, expandedkey, &ciphertext[offset * 16]);
    //Print Output Block
    printf("\nciphertext of block %d : ", k+1);
    for(i =0 ;i< 16; i++){
	...
	//Print Input Block
        ...
        aes_decrypt(key, expandedkey, &ciphertext[k*16], block_message);
	XOR(block_message, round_iv);
	//Print Output Block
        printf("\nOutput block %d : ", k+1);
        for(i =0 ;i< 16; i++){
