...
void aes_encrypt_rounds(unsigned char *state, unsigned char *ciphertext,unsigned char *expandedkey){
    ...
    //print(state);
    //printf("\nRound 0:\n"); 
    generate_roundkey(roundkey, &expandedkey[0]);
	...
	//print(state);
        mixcolumn(state);
	//print(state);
	generate_roundkey(roundkey, &expandedkey[i*16]);
        Add_roundkey(state, roundkey);
	//print(state);
    }
    //printf("\nRound 10:\n");
    ...
    shift_row(state);
    //print(state);
    generate_roundkey(roundkey, &expandedkey[160]);
    Add_roundkey(state, roundkey);    
...
void aes_decrypt(unsigned char *key, unsigned char *expandedkey, unsigned char *ciphertext, unsigned char *decryptedtext){
    ...
    //AES decryption
    aes_decrypt_rounds(state, decryptedtext, expandedkey);
     
}

void aes_decrypt_rounds(unsigned char *state, unsigned char *DecryptedText, unsigned char *expandedkey){
    unsigned char roundkey[16];
    //printf("\nRound 0: \n");
    generate_roundkey(roundkey, &expandedkey[160]);
    Add_roundkey(state, roundkey);
